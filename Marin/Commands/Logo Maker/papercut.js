...
module.exports = {
    name: "papercut",
    alias: ["pcut"],
    desc: "Make Papercut Style Text Logo",
    category: "Logo Maker",
    react: "🍁",

    start: async (Miku, m, { prefix, text }) => {
        if (!text) return m.reply(`Example: *${prefix}papercut Marin-MD Bot*`)
        ...
        try {
            ...
            // --- PIXABAY RANDOM BACKGROUND (papercut / abstract) ---
            const pixabayAPI = "54164246-c83b8dee398b874d43650c040"
            const query = "papercut,abstract,colorful"
            ...
            console.error(err)
            m.reply("Error while generating papercut logo.")
        }
    }
}
