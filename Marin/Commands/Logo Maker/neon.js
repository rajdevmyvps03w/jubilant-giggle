...
module.exports = {
    name: "neon",
    alias: ["neonlogo"],
    desc: "Make Neon Style 3D Text Logo with random API background",
    category: "Logo Maker",
    react: "🍁",

    start: async (Miku, m, { prefix, text }) => {
        if (!text) return m.reply(`Example: *${prefix}neon Marin-MD Bot*`)
        ...
        try {
            ...
            const query = "neon,night,city" // random neon/city style
            ...
            console.error(err)
            m.reply("Error while generating neon logo.")
        }
    }
}
